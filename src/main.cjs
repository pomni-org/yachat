...
async function handleHttpRequest(request, response) {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      if (request.method === "POST" && url.pathname === "/api/chat/block") {
        sendJson(response, 200, await localBackend.blockUser(await readRequestJson(request)));
        return;
      }

      if (request.method === "POST" && url.pathname === "/api/chat/unblock") {
        sendJson(response, 200, await localBackend.unblockUser(await readRequestJson(request)));
        return;
      }
...
ipcMain.handle("chat:block-user", async (_event, payload) => {
  return localBackend.blockUser(payload);
});

ipcMain.handle("chat:unblock-user", async (_event, payload) => {
  return localBackend.unblockUser(payload);
});
