...
function createLocalBackend(app, appTitle) {
  ...
  function chatBlockState(chat, account, participantIds = chatParticipantIds(chat)) {
    if (chat?.kind !== "private" || !account?.id) {
      return { blockedByMe: false, blockedMe: false, peerId: "" };
    }
    const peerId = participantIds.find((id) => id !== account.id) || "";
  ...
  function summarizeChat(chat, messages, account = null, usersById = new Map()) {
    const list = messagesForAccount(messages[chat.id] || [], account);
    ...
    const blockState = chatBlockState(chat, account, participantIds);
  ...
  function messagesForAccount(messages, account) {
    ...
    return (Array.isArray(messages) ? messages : [])
      .filter((message) => !accountId || !(Array.isArray(message?.hiddenFor) && message.hiddenFor.map(String).includes(accountId)))
      .map((message) => messageForAccount(message, account));
  }
  ...
  async function setChatUserBlocked(payload, blocked) {
    const state = await ensureMessengerState();
    const account = await getLastAccount();
    const usersById = createUserMap(await readAllManifests());
    const chat = state.chats.find((item) => item.id === payload?.chatId);
    ...
    if (!chat || chat.kind !== "private" || !account?.id) {
      throw new Error("Блокировка доступна только в личных чатах.");
    }

    const blockState = chatBlockState(chat, account);
    ...
    chat.updatedAt = new Date().toISOString();
    await saveMessengerState(state);
    const summary = summarizeChat(chat, state.messages, account, usersById);
  ...
  async function blockUser(payload) {
    return setChatUserBlocked(payload, true);
  }

  async function unblockUser(payload) {
    return setChatUserBlocked(payload, false);
  }
  ...
  async function sendMessage(payload) {
    ...
    if (blockState.blockedByMe || blockState.blockedMe) {
      throw new Error("Сначала разблокируйте пользователя, чтобы отправить сообщение.");
    }
  ...
  async function deleteMessage(payload) {
    ...
    if (scope === "everyone") {
      ...
      selected.forEach((message) => {
        message.hiddenFor = [...new Set([...(Array.isArray(message.hiddenFor) ? message.hiddenFor : []), account.id])];
      });
    }
    await saveMessengerState(state);
    return {
  ...
  async function forwardMessage(payload) {
    ...
    const blockState = chatBlockState(toChat, account);
    if (blockState.blockedByMe || blockState.blockedMe) {
      throw new Error("Сначала разблокируйте пользователя, чтобы отправить сообщение.");
    }
