(() => {
  ...
  async function encodePhotoWithoutBlocking(file) {
    ...
    try {
      ...
      const preservePng = file.type === "image/png" && !spoiled && file.size <= 2.5 * 1024 * 1024;
      const type = preservePng ? "image/png" : "image/jpeg";
      let blob = await canvasToBlob(canvas, type, spoiled ? 0.68 : 0.86);
      if (!preservePng && blob.size > 3.2 * 1024 * 1024) {
        await nextFrame();
        blob = await canvasToBlob(canvas, "image/jpeg", spoiled ? 0.54 : 0.7);
      }

      return { dataUrl: await blobToDataUrl(blob), spoiled };
    } finally {
      source.close?.();
    }
  }
